...
for(DaoViewSensorListRecord rVSL : aryViewSensor)
{
	...
	joSensor.put("fixed", rVSL.m_nFixed);
	
	if(rVSL.m_strKey.equals("d")) joSensor.put("key2", "ad");
	else if(rVSL.m_strKey.equals("p")) joSensor.put("key2", "ap");
	else if(rVSL.m_strKey.equals("u")) joSensor.put("key2", "au");
	else if(rVSL.m_strKey.equals("n")) joSensor.put("key2", "an");
	else if(rVSL.m_strKey.equals("s")) joSensor.put("key2", "as");	
    ...
    <head>
		...
		*/
		...
		function drawChart() 
		{
			...
			$.getJSON("./query/query_graph_air.jsp",sParam,function(data,status){
			if(status == "success")
			{
				if(data.result	== "OK")
				{
		...
		function drawOneChart(strKey, bPopup)
		{
			...
			for(var nIndex = 0; nIndex < j_jaSensor.length ; nIndex++)
			{
				if(j_jaSensor[nIndex].key == strKey)
				{
					...
					if(j_jaSensor[nIndex].fixed == 0)strFixed = "#";
					else if(j_jaSensor[nIndex].fixed == 3)strFixed = "#.###";
				}
			}
			...
			if(strKey2 == "ad") dataGraph.addColumn('number', "기상청 " + strName);   //노면온도 가져오는데만 에어코리아이기 떄문
			else if(strKey2) dataGraph.addColumn('number', "환경부 " + strName);
		...
		function SensorChange(seleteSensor)
		{
			...
			var tempTh1 = "<tr><th rowspan='2'>날짜</th>";
			var tempTh2 = "<tr>";
			...
			if(strSensorType == "all")
			{
				for(var nIndex = 0; nIndex < j_jaSensor.length ; nIndex++)
				{
					if( (j_jaSensor[nIndex].keyindex & j_jaSList[nSensorIndex].viewsensor) == j_jaSensor[nIndex].keyindex)
					{
						if(j_jaSensor[nIndex].key2  == "ad") //노면온도 가져오는데만 에어코리아이기 떄문
						{
							tempCol += "<col style='width:3%'>";
							tempCol += "<col style='width:3%'>";
							tempTh1 += "<th colspan='2'>"+j_jaSensor[nIndex].name+"</th>";
							tempTh2 += "<th>센서</th><th>기상청</th>";
						}
						else if(j_jaSensor[nIndex].key2)
						{
							tempCol += "<col style='width:3%'>";
							tempCol += "<col style='width:3%'>";
							tempTh1 += "<th colspan='2'>"+j_jaSensor[nIndex].name+"</th>";
							tempTh2 += "<th>센서</th><th>환경부</th>";
						}
						else
						{
							tempCol += "<col style='width:5%'>";
							tempTh1 += "<th rowspan='2'>"+j_jaSensor[nIndex].name+"</th>";
						}
					}
			...
			$("#id_list_table_head >thead:last").append(tempTh1);
			$("#id_list_table_head >thead:last").append(tempTh2);
...
//						fExpData = Math.abs(parseFloat(jM[j_jaSensor[nIndex].key]));
						fExpData = parseFloat(jM[j_jaSensor[nIndex].key]);
						if(j_jaSensor[nIndex].fixed == 0) fExpData = fExpData.toFixed(0);
...
//							fExpData = Math.abs(parseFloat(jM[j_jaSensor[nIndex].key2]));
							fExpData = parseFloat(jM[j_jaSensor[nIndex].key2]);
							if(j_jaSensor[nIndex].fixed == 0) fExpData = fExpData.toFixed(0);
							else  fExpData = fExpData.toFixed(3);
		...
		function fnExcelReport() {
			...
			{
				if( (j_jaSensor[nIndex].keyindex & j_jaSList[nSensorIndex].viewsensor) == j_jaSensor[nIndex].keyindex)
				tab_head += '<th>' + j_jaSensor[nIndex].name + '</th>';   
			
				if(j_jaSensor[nIndex].key2)
				tab_head += '<th>기상청' + j_jaSensor[nIndex].name + '</th>';		
			}
			...
			var fileName = 'enclosureReport.xls';
			//Explorer 환경에서 다운로드
       ...
       <div  style="min-width: 400px" >
			...
			<jsp:include page="./header.jsp"></jsp:include> 	 	
             ...
             <div class="container" >
                 ...
                 <h3 class="tit-list">환경 스테이션 리포트</h3>
